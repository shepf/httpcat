...
| macOS Apple Silicon | `darwin_arm64` | M1/M2/M3/M4 Mac | ✅ |
| Windows x64 | `windows_amd64` | Windows 系统 | ✅ |
| macOS Intel | `darwin_amd64` | Intel Mac (已淘汰) | ❌ 需手动指定 |
...
构建完成后，`release/` 目录结构（默认 4 个平台）：

```
release/
├── httpcat_v0.7.0_linux-amd64.tar.gz    # Linux x86_64 安装包
├── httpcat_v0.7.0_linux-arm64.tar.gz    # Linux ARM64 安装包
├── httpcat_v0.7.0_darwin-arm64.tar.gz   # macOS Apple Silicon 安装包
└── httpcat_v0.7.0_windows-amd64.zip     # Windows 安装包
```
...
| **Linux** | Windows | ❌ | ❌ | **使用 Docker** |
| **macOS** | macOS (本机) | ✅ | ✅ | 直接编译 |
| **macOS** | Linux | ❌ | ❌ | **使用 Docker** |
| **macOS** | Windows | ❌ | ❌ | **使用 Docker** |
...
在 macOS 上交叉编译 Linux/Windows 版本时，**脚本会自动检测并建议使用 Docker**：
...
# 方式 2：明确使用 Docker 构建所有平台（推荐）
./scripts/build.sh -d -a -f

# 方式 3：只构建 macOS 版本（不需要 Docker）
./scripts/build.sh -p darwin_arm64 -f
```

**注意**：如果在 macOS 上不使用 Docker 构建 Linux/Windows 版本，编译产物将**无法登录**（无 SQLite 支持）。

### Docker 构建说明

Docker 构建使用以下工具链：

| 目标平台 | Docker 基础镜像 | 编译工具 |
|---------|---------------|---------|
| Linux amd64/arm64 | golang:1.23-alpine | musl-dev + sqlite-dev |
| Windows amd64 | golang:1.23-bookworm | mingw-w64 |

**静态链接**：Linux 版本使用静态链接，不依赖系统库，兼容所有 Linux 发行版（Ubuntu、CentOS、Debian 等）
...
```bash
# 解压安装包
tar -xzf httpcat_v0.7.0_linux-amd64.tar.gz
cd httpcat_v0.7.0_linux-amd64
...
```bash
./httpcat -v
# 输出: httpcat version v0.7.0 (build: 202401311200, commit: abc1234)
```
