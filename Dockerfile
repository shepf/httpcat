...
RUN CGO_ENABLED=1 GOOS=linux CGO_CFLAGS="-D_LARGEFILE64_SOURCE" go build \
    -ldflags "-s -w -X gin_web_demo/server/common.Version=${VERSION} -X gin_web_demo/server/common.Build=${BUILD_TIME} -X gin_web_demo/server/common.Commit=${COMMIT_ID}" \
...
# 从编译阶段复制二进制文件
COPY --from=builder /app/httpcat /app/httpcat
